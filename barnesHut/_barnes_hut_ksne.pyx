...
cdef float compute_gradient(float[:] val_P,
    ...
    if qt.verbose > 11:
        printf("[k-sne] Allocating %li elements in force arrays\n",
    ...
    if take_timing:
        t2 = clock()
        printf("[k-sne] Computing negative gradient: %e ticks\n", ((float) (t2 - t1)))
    ...
    if take_timing:
        t2 = clock()
        printf("[k-sne] Computing positive gradient: %e ticks\n",
               ((float) (t2 - t1)))
...
cdef float compute_gradient_positive(float[:] val_P,
    ...
    if verbose > 10:
        t2 = clock()
        dt = ((float) (t2 - t1))
        printf("[k-sne] Computed error=%1.4f in %1.1e ticks\n", C, dt)
    return C
...
cdef double compute_gradient_negative(float[:, :] pos_reference,
    ...
    with nogil, parallel(num_threads=num_threads):
        ...
        for i in prange(start, n_samples, schedule='static'):
            ...
            if take_timing:
                t2 = clock()
            # Compute the k-sne negative force
        ...
        free(summary)
    if take_timing:
        printf("[k-sne] Tree: %li clock ticks | ", dta)
...
def gradient(float[:] val_P,
    ...
    if verbose > 10:
        printf("[k-sne] Initializing tree of n_dimensions %i\n", n_dimensions)
    cdef _QuadTree qt = _QuadTree(pos_output.shape[1], verbose)
    if verbose > 10:
        printf("[k-sne] Inserting %li points\n", pos_output.shape[0])
    ...
    if verbose > 10:
        ...
        printf("[k-sne] Computing gradient\n%s", EMPTY_STRING)
    ...
    if verbose > 10:
        ...
        # and -Werror=format-security
        printf("[k-sne] Checking tree consistency\n%s", EMPTY_STRING)
