...
def ksne_v_backprop(network, train_X, train_labels, test_X, test_labels, max_iter=30, perplexity=30, v=None):
    if v is None:
        v = network[-1]['bias_upW'].shape[1] - 1
    ...
    for batch in range(0, n, batch_size):
        if batch + batch_size <= n:
            ...
            p, _ = x2p(cur_batch, perplexity, 1e-5)
            p[np.isnan(p)] = 0
            p = (p + p.T) / 2
            p /= np.sum(p)
