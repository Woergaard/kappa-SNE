...
def _binary_search_perplexity(sqdistances, desired_perplexity, verbose):
    ...
    for i in range(n_samples):
        ...
        if verbose and ((i + 1) % 1000 == 0 or i + 1 == n_samples):
            print("[k-SNE] Computed conditional probabilities for sample "
                  "%d / %d" % (i + 1, n_samples))
    
    if verbose:
        print("[k-SNE] Mean sigma: %f"
...
def _kl_divergence(params, P, k, beta, n_samples, n_components,
    ...
    P : array, shape (n_samples * (n_samples-1) / 2,)
        Condensed joint probability matrix.
    k : float
        Hyperparameter of the Kaniadakis-Gaussian distribution.
    n_samples : int
        Number of samples.
    n_components : int
    ...
    # Q is based on Kaniadakis-Gaussian distribution with hyperparameter k
    dist = pdist(X_embedded, "sqeuclidean")                                                                           
    
    tmp = (np.sqrt(1.+(k**2.)*(beta**2.)*(dist**2.))-k*beta*dist)
    prob = tmp**(1./k)
...
def _gradient_descent(objective, p0, it, n_iter,
                      ...
                      min_grad_norm=1e-7, verbose=1, args=None, kwargs=None):
...
#            print("grad", grad)
        ...
        if check_convergence:
            ...
            if verbose >= 2:
                print("[k-SNE] Iteration %d: error = %.7f,"
                      " gradient norm = %.7f"
            ...
            if error < best_error:
                ...
                if verbose >= 2:
                    print("[k-SNE] Iteration %d: did not make any progress "
                          "during the last %d episodes. Finished."
                          % (i + 1, n_iter_without_progress))
                break
            if grad_norm <= min_grad_norm:
                if verbose >= 2:
                    print("[k-SNE] Iteration %d: gradient norm %f. Finished."
                          % (i + 1, grad_norm))
                break
...
class kSNE(BaseEstimator):
    ...
    k : float, optional (default: 0.5)
        The k is the hyperparameter of Kaniadakis-Gaussian distribution.
        The range of k is between 0<k<1.
    beta : float, optional (default: 1.0)
        Shape parameter for the Kaniadakis-Gaussian distribution. 
        It requires beta to be greater than 0.
    ...
    def _fit(self, X, skip_num_points=0):
        ...
        if self.metric == "precomputed":
            ...
            check_non_negative(X, "kSNE.fit(). With metric='precomputed', X "
                                  "should contain positive distances.")

            if issparse(X):
                raise TypeError(
                    'kSNE does not accept sparse '
        ...
        return self._ksne(P, self.k, self.beta, n_samples,
                          X_embedded=X_embedded,
                          neighbors=neighbors_nn,
                          skip_num_points=skip_num_points)

    def _ksne(self, P, k, beta, n_samples, X_embedded,
              neighbors=None, skip_num_points=0):
        """Runs k-SNE."""
        # k-SNE minimizes the Kullback-Leiber divergence of the Gaussians P
        # and the Kaniadakis-Gaussian Q. The optimization algorithm that
        ...
        params, kl_divergence, it = _gradient_descent(obj_func, params,
                                                      **opt_args)
        if self.verbose:
            print("[k-SNE] KL divergence after %d iterations with early "
        ...
        if self.verbose:
            print("[k-SNE] KL divergence after %d iterations: %f"
                  % (it + 1, kl_divergence))
